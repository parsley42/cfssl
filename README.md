...
This repository and associated releases are a special-purpose build of cfssl that includes an `ssl-pairgen` script and only the required **cfssl** binaries for generating user private client pkcs#12 bundles and certificate authorities, for use in a [Kubernetes](https://kubernetes.io) cluster.
...
## ssl-pairgen END
What follows is the original README from **cfssl**, the excellent ssl / ca package from which **ssl-pairgen** was derived (or, more accurately, cobbled).
